...
const double _kToolbarHeight = 120.0;
...
class _TextSelectionToolbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<Widget> items = <Widget>[
      _IconButton(icon: Icons.brush, title: 'Highlight', onTap: onHighlight),
      _IconButton(icon: Icons.note_add, title: 'Note'),
      _IconButton(icon: Icons.bookmark_border, title: 'Bookmark'),
    ];
      ...
      child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Padding(
            ...
            child: Icon(Icons.volume_up, color: Constants.red, size: 35.0),
          ),
          Row(
        ...
        decoration: BoxDecoration(
            border: Border(top: BorderSide(color: Constants.red, width: 2.0))),
      ),
    );
  }
}
...
class _IconButton extends StatelessWidget {
  ...
  const _IconButton({Key key, this.icon, this.title, this.onTap})
      : super(key: key);
...
class ReaderTextSelectionControls extends TextSelectionControls {
  final VoidCallback onHighlight;

  ReaderTextSelectionControls({this.onHighlight});
